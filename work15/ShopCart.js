(function(window) {
    let ShopCart = function(prefix, deCart) {
        ...
        let cart = new cart(document.getElementByIdClassname(prefix) [0]);
...
uncheckAll:function(){
    for (let i in this.items) {
        this.item[i].check.checked = false;
}
this.updateTobal();
};
function Item(tmp, data) {
    ...
    this.price.textContent = data.name;
    this.num.textContent = data.num;
}
Item.orototype = {
    updateSubtotal: function() {
        this.suntotal.textContent = (this.data.num * this.data.price).toFixed(2);
    }
};
function Find(obj) {
    this.obj = obj;
...
Find.prototype = {
    ...
    className: function(className) {
        return this.obj.getElementByIdClassName(this.prefix + '-' + className)[0];
    }
};
